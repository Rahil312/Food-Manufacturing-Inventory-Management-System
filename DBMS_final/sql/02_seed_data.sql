...
-- NOTE: This file assumes the database and tables have been created via 01_schema_and_logic.sql
...
-- Using plain text 'password123' for demo purposes
...
-- ------------------------------------------------------------------
-- NOTE: We need to manually insert these since they represent historical production
...
-- NOTE: The after-insert trigger will automatically update on_hand quantities
...
/*
SELECT 'manufacturer' as table_name, COUNT(*) as record_count FROM manufacturer
UNION ALL SELECT 'supplier', COUNT(*) FROM supplier
UNION ALL SELECT 'category', COUNT(*) FROM category
UNION ALL SELECT 'user_account', COUNT(*) FROM user_account
UNION ALL SELECT 'ingredient', COUNT(*) FROM ingredient
UNION ALL SELECT 'ingredient_material', COUNT(*) FROM ingredient_material
UNION ALL SELECT 'supplier_ingredient', COUNT(*) FROM supplier_ingredient
UNION ALL SELECT 'supplier_formulation', COUNT(*) FROM supplier_formulation
UNION ALL SELECT 'supplier_formulation_material', COUNT(*) FROM supplier_formulation_material
UNION ALL SELECT 'do_not_combine', COUNT(*) FROM do_not_combine
UNION ALL SELECT 'product_type', COUNT(*) FROM product_type
UNION ALL SELECT 'recipe_plan', COUNT(*) FROM recipe_plan
UNION ALL SELECT 'recipe_plan_item', COUNT(*) FROM recipe_plan_item
UNION ALL SELECT 'ingredient_batch', COUNT(*) FROM ingredient_batch
UNION ALL SELECT 'product_batch', COUNT(*) FROM product_batch
UNION ALL SELECT 'product_batch_consumption', COUNT(*) FROM product_batch_consumption;
*/

-- Verify ingredient batch lot numbers (generated by trigger)
/*
SELECT ingredient_batch_id, lot_number, ingredient_id, supplier_id, 
       quantity_oz, on_hand_oz, expiration_date
FROM ingredient_batch
ORDER BY ingredient_id, supplier_id;
*/

-- Verify product batch traceability
/*
SELECT pb.product_lot_number, pb.produced_units, pb.batch_cost, pb.unit_cost,
       ib.lot_number as ingredient_lot, i.name as ingredient_name, 
       pbc.qty_oz as consumed_qty
...
ORDER BY pb.product_batch_id, i.ingredient_id;
*/

-- Verify on-hand quantities after consumption
/*
SELECT i.name, ib.lot_number, ib.quantity_oz, ib.on_hand_oz, 
       (ib.quantity_oz - ib.on_hand_oz) as consumed_oz
FROM ingredient_batch ib
JOIN ingredient i ON i.ingredient_id = ib.ingredient_id
ORDER BY i.ingredient_id;
*/

-- Verify do-not-combine conflicts
/*
...
SELECT 'Run verification queries above to confirm data integrity' as next_step;
